...
public class ArraysEx1 {
    public static void main(String[] args) {
        ...
        //얕은 복제 : 다른 곳(기억장소)에 같은 데이터를 복제하는 것이 아니라 이미 저장되어 있는 데이터의 주소만을 다른 변수에 저장
        ...
        System.out.println(Arrays.equals(arr1,arr3));   //배열의 값을 비교하려면 Arrays.equals(,)를 써야 함

        System.out.println("정렬 전");
        int[] points = {90,100,80,70,95,85,75};
        for(int p:points){    //정련 전
            System.out.println(p);
        }
        System.out.println("정렬 후");
        Arrays.sort(points);  //정렬 후
        for(int p:points){
            System.out.println(p);
        }
